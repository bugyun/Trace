...
public class LoggerFactory implements Factory {
    ...
    private static final int DEBUG = Trace.LogLevel.DEBUG;
    private static final int ERROR = Trace.LogLevel.ERROR;
    private static final int ASSERT = Trace.LogLevel.ASSERT;
    private static final int INFO = Trace.LogLevel.INFO;
    private static final int VERBOSE = Trace.LogLevel.VERBOSE;
    private static final int WARN = Trace.LogLevel.WARN;
    /**
    ...
    public synchronized void log(int priority, String tag, String message, Throwable throwable) {
        if (settings.getLogLevel() <= Trace.LogLevel.NONE) {
            return;
        }
    ...
    private void logChunk(int logType, String tag, String chunk) {
        ...
        switch (logType) {
            case VERBOSE:
                if (settings.getLogLevel() >= VERBOSE)
            ...
            case INFO:
                ...
                break;
            case WARN:
                if (settings.getLogLevel() >= WARN)
                    settings.getLogAdapter().w(finalTag, chunk);
                break;
            case ERROR:
                if (settings.getLogLevel() >= ERROR)
                    settings.getLogAdapter().e(finalTag, chunk);
                break;
            case ASSERT:
                if (settings.getLogLevel() >= ASSERT)
...
//                settings.getLogAdapter().d(finalTag, chunk);
    ...
    private int getStackOffset(StackTraceElement[] trace) {
        for (int i = MIN_STACK_OFFSET; i < trace.length; i++) {
            ...
            if (!name.equals(LoggerFactory.class.getName()) && !name.equals(Logger.class.getName())) {
                return --i;
            }
        }
        return -1;
